...
- `username` _{string}_ - The user's username
- `password` _{string}_ - The user's password
- `depolarize` _{boolean}_ - If user wants to depolarize their feed (remove freets with more downvotes than likes)
...
#### `DELETE /api/like?freetId=FREETID` - Delete an existing like 
...
#### `GET /api/follow/followers?followee=FOLLOWEE` - Gets the follow objects for people who are following the followee

**Returns**

- Array of follow objects who are following the followee that was specified

**Throws** 

- `400` If the followee param is not passed
- `404` If no user with the followee username exists

#### `GET /api/follow/following?user=USERNAME` - Gets the follow objects for people who the user is following

**Returns**

- Array of follow objects who the user is following

**Throws** 

- `400` If the user param is not passed
- `404` If no user with the user's username exists

#### `GET /api/follow/feed?user=USERNAME` - Gets the feed for a user (freets from people they user follows). If the user has depolarize turned on, the feed will not show freets that have more downvotes than likes.


**Returns**

- An array of freets from people the user follows
...
- Follower: username of the usr who is following another user
...
- `403` if user isn’t logged in or if the follow object already exists

#### `DELETE /api/follow?follower=FOLLOWER&followee=FOLLOWEE` - Deletes the follow object corresponding to the specified fields
...
- `403` if user isn’t logged in or if the follow object doesn't exist
...
#### `POST /api/downvotes` - Create a new downvote object
...
#### `DELETE /api/downvotes?freetId=FREETID` - Delete an existing downvote 
...
#### `GET /api/article` - Get all verified news articles
...
#### `POST /api/article` - Add a new verified news article object
...
#### `PATCH /api/article/:articleId` - Edit an existing verified news object
...
- `403` if not logged in as admin account
- `404` i the articleId is not valid
